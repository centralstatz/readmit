...
- **Reporting lineage**: It is often difficult to have seamless line of
  ...
  about readmissions for all patients).  
...
- **Payor Contracts/Reimbursement**: Readmissions have different
  ...
  prevention strategies while optimizing financial health.  
...
- **Oversaturation of research**: Readmissions has a [large body of
  ...
  program for any one hospital, especially when it’s conflicting.  
...
For example, `hrrp_performance_periods` provides the date ranges for
discharges that are included in each program year:
...
function to extract relevant time periods for an inputted date. For
example:
...
straight from the website into clean datasets in `R` (see
...
to get the collection of topics seen
[here](https://data.cms.gov/provider-data/):
